...
import { Channel, ChannelContract, ChannelSettlementRecord } from '../../types';
import { transitionChannelSettlementStatus, appendDisputeNote } from '../../lib/settlementService';
...
export function ChannelSettlementDetailPage({
  ...
  const handleAction = async () => {
    ...
    setSaving(true);
    try {
      let updated: ChannelSettlementRecord;
      if (activeAction === 'start_reconciliation') {
        updated = await transitionChannelSettlementStatus(record.id, 'IN_RECONCILIATION');
        ...
        updated = await transitionChannelSettlementStatus(record.id, 'SETTLED', {
        ...
        updated = await transitionChannelSettlementStatus(record.id, 'DISPUTED', {
          notes: noteInput,
          operator: 'ops-admin',
        });
        ...
        updated = await appendDisputeNote('channel', record.id, noteInput) as ChannelSettlementRecord;
      } else {
        setSaving(false);
        return;
      }
